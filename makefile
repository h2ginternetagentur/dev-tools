...
gitAddCommit=/usr/local/bin/git-commit
helpFile=/usr/local/bin/dev-tools-help

install: remove-old install-git-push install-git-commit install-git-delete-branches install-doc-index install-doc-file-index h2g-alias dev-tools-help info
info:
...
dev-tools-help:
	...
	chmod 677 $(helpFile)
remove-old:
	rm -rf /usr/local/bin/push
	rm -rf /usr/local/bin/commit
